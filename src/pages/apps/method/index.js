...
// ** Actions Imports
import { fetchData } from 'src/store/apps/method'

// ** Third Party Components
import axios from 'axios'

// ** Custom Table Components Imports
import TableHeader from 'src/views/apps/method/tableHeader'
import AddBankDrawer from 'src/views/apps/method/addDrawer'
...
// console.log("storemethod",store.method)
  ...
  const columns = [
    {
      flex: 0.1,
      minWidth: 150,
      field: 'name', // Replace with the actual field name for Method Name
    ...
    },
    {
      flex: 0.15,
      minWidth: 190,
      field: 'createdAt', // Replace with the actual field name for Added Date
...
const BankList = ({ apiData }) => {
  ...
  // ** Hooks
  const dispatch = useDispatch()
  const store = useSelector(state => state.method)
  // console.log("store",state.method)
  useEffect(() => {
    dispatch(
      fetchData({
        role,
  ...
  return (
    <Grid container spacing={6.5}>
      <Grid item xs={12}>
        <Card>
          ...
          <DataGrid
            autoHeight
            rowHeight={62}
            rows={store.method && store.method.length ? store.method : []}
            getRowId={(row) => hashCode(JSON.stringify(row))} // Provide fallback properties
      ...
      <AddBankDrawer
        ...
        methodData={editUserOpen ? methodData : undefined}
