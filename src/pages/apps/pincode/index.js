...
// ** Actions Imports
import { fetchData } from 'src/store/apps/pincode'

// ** Third Party Components
import axios from 'axios'

// ** Custom Table Components Imports
import TableHeader from 'src/views/apps/pincode/TableHeader'
import AddBankDrawer from 'src/views/apps/pincode/addDrawer'
...
const getCols = toggleEditUserDrawer => {
  const columns = [
    ...
    
    {
      ...
      renderCell: ({ row }) => (
        <img
          src={row.file} // Assuming row.file contains the URL to the image
          alt="File Preview"
          style={{ maxWidth: '100%', maxHeight: '100%', borderRadius: '5px' }} // Customize the styles as needed
        />
      ),
    },
    {
...
const BankList = ({ apiData }) => {
  ...
  // ** Hooks
  const dispatch = useDispatch()
  const store = useSelector(state => state.pincode)
  useEffect(() => {
    dispatch(
      fetchData({
        role,
  ...
  return (
    <Grid container spacing={6.5}>
      <Grid item xs={12}>
        <Card>
          ...
          <DataGrid
            autoHeight
            rowHeight={62}
            rows={store.pincode && store.pincode.length ? store.pincode : []}
      ...
      <AddBankDrawer
        ...
        pincodeData={editUserOpen ? pincodeData : undefined}
