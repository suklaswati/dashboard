...
// ** Actions Imports
import { fetchData } from 'src/store/apps/rateunit'

// ** Third Party Components
import axios from 'axios'

// ** Custom Table Components Imports
import TableHeader from 'src/views/apps/rateunit/tableheader'
import AddBranchDrawer from 'src/views/apps/rateunit/addDrawer'
...
const getCols = toggleEditUserDrawer => {
  const columns = [
    ...
    },
    {
      flex: 0.15,
      minWidth: 190,
      field: 'createdAt', // Assuming 'createdAt' corresponds to the Rate Unit Added Date
      headerName: 'Rate Unit Added Date',
      renderCell: ({ row }) => (
        <Typography noWrap sx={{ color: 'text.secondary' }}>
...
const BankBranchList = ({ apiData }) => {
  ...
  // ** Hooks
  const dispatch = useDispatch()
  const store = useSelector(state => state.rateunit)
  useEffect(() => {
    dispatch(
      fetchData({
        role,
  ...
  const [rateunitData, setrateunitData] = useState({})
  ...
  const handleRow = data => {
    setrateunitData(data)
    console.log("hello", rateunitData)
  }
  ...
  return (
    <Grid container spacing={6.5}>
      <Grid item xs={12}>
        <Card>
          ...
          <DataGrid
            autoHeight
            rowHeight={62}
            rows={store.rateunit && store.rateunit.length ? store.rateunit : []}
      ...
      <AddBranchDrawer
        ...
        rateunitData={editUserOpen ? rateunitData : undefined}
      />
    </Grid>
  )
}
