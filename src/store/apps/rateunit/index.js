...
export const fetchData = createAsyncThunk('fetchgstdata', async (params, { getState, dispatch }) => {
    ...
    try {
        ...
        const response = await axios.get(`https://habitus-admin-api.applore.in/api/v1/rate-unit/rateUnits?search=${params?.q}`, {
            headers,
        })
...
export const branchSlice = createSlice({
    ...
    extraReducers: builder => {
        builder.addCase(fetchData.fulfilled, (state, action) => {
            ...
            state.rateunit = action.payload.data.rateunit
        })
    }
})
